...
def display_all_charts():
    # Line Chart: Smoking Prevalence Over Time
    ...
    # Bar Chart: Smoking Prevalence by Age Group
    ...
    # Box Plot: Drug Experimentation Distribution by Age Group
    ...
    ax.pie(gender_data['Count'], labels=gender_data['Gender'], autopct='%1.1f%%', startangle=90, colors=sns.color_palette('Set3', len(gender_data)))
    ax.axis('equal')  # Equal aspect ratio ensures that pie chart is drawn as a circle.
    st.pyplot(fig)

    # Heatmap: Peer Influence vs Smoking Prevalence by Year
    ...
    # Histogram: Mental Health vs Drug Experimentation by Gender
    ...
    # Scatter Plot with Line: Age Group vs Peer Influence
    st.subheader("Scatter Plot with Line: Age Group vs Peer Influence")
    st.write("""
    ...
    scatter_plot_age_peer = alt.Chart(filtered_data).mark_point().encode(
        x='Age_Group:O',  # Categorical variable for Age Group
        y='Peer_Influence:Q',  # Continuous variable for Peer Influence
        color='Gender:N',  # Color points by gender
        tooltip=['Age_Group', 'Peer_Influence', 'Gender']  # Tooltips for interactive info
    ).properties(width=700, height=400)

    line_age_peer = alt.Chart(filtered_data).mark_line().encode(
        x='Age_Group:O',
        y='Peer_Influence:Q',
        color=alt.value('black')  # Black line for trend
    ).transform_regression('Age_Group', 'Peer_Influence')

    st.altair_chart(scatter_plot_age_peer + line_age_peer)

# Show all charts if selected
if show_all_charts:
    display_all_charts()
else:
    if chart_type == "Line Chart":
        st.subheader("Line Chart: Smoking Prevalence Over Time")
        st.write("The line chart tracks smoking prevalence over a range of years...")
        line_chart = alt.Chart(filtered_data).mark_line().encode(
            x='Year:O',
            y='Smoking_Prevalence:Q',
            color='Gender:N'
        ...
        st.subheader("Bar Chart: Smoking Prevalence by Age Group")
        st.write("The bar chart visualizes the average smoking prevalence across different age groups...")
        age_group_bar = filtered_data.groupby('Age_Group')['Smoking_Prevalence'].mean().reset_index()
        bar_chart = alt.Chart(age_group_bar).mark_bar().encode(
            x='Age_Group:O',
            y='Smoking_Prevalence:Q',
        ...
        st.subheader("Pie Chart: Gender Distribution")
        st.write("The pie chart visualizes the distribution of youth across different gender categories...")
        gender_data = filtered_data['Gender'].value_counts().reset_index()
        gender_data.columns = ['Gender', 'Count']
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.pie(gender_data['Count'], labels=gender_data['Gender'], autopct='%1.1f%%', startangle=90, colors=sns.color_palette('Set3', len(gender_data)))
        ...
        st.subheader("Box Plot: Drug Experimentation Distribution by Age Group")
        st.write("The box plot shows the distribution of drug experimentation across different age groups...")
        box_plot = alt.Chart(filtered_data).mark_boxplot().encode(
            x='Age_Group:O',
            y='Drug_Experimentation:Q',
            color='Age_Group:N'
        ...
        st.subheader("Heatmap: Peer Influence vs Smoking Prevalence")
        st.write("The heatmap highlights the relationship between peer influence and smoking prevalence over time...")
        heatmap_data = filtered_data.groupby(['Year', 'Peer_Influence'])['Smoking_Prevalence'].mean().reset_index()
        heatmap_pivot = heatmap_data.pivot(index='Year', columns='Peer_Influence', values='Smoking_Prevalence')
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.heatmap(heatmap_pivot, annot=True, cmap="YlGnBu", ax=ax, linewidths=0.5)
        ...
        st.subheader("Histogram: Mental Health vs Drug Experimentation")
        st.write("The histogram shows the distribution of mental health scores in relation to drug experimentation...")
        hist = alt.Chart(filtered_data).mark_bar().encode(
            x=alt.X('Mental_Health:Q', bin=True),
            y='count():Q',
            color='Gender:N'
        ...
        st.subheader("Scatter Plot: Mental Health vs Drug Experimentation")
        st.write("The scatter plot shows the relationship between mental health and drug experimentation...")
        scatter_plot = alt.Chart(filtered_data).mark_point().encode(
            x='Mental_Health:Q',
            y='Drug_Experimentation:Q',
            color='Gender:N',
            tooltip=['Mental_Health', 'Drug_Experimentation', 'Gender']
        ).properties(width=700, height=400)
